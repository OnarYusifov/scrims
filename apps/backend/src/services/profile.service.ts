...
export interface ProfileResponseOptions {
...
export async function fetchProfileUser(userId: string): Promise<ProfileUser | null> {
  ...
  }) as Promise<ProfileUser | null>;
}

export async function fetchProfileUserByDiscordId(discordId: string): Promise<ProfileUser | null> {
  return prisma.user.findUnique({
    where: { discordId },
    select: profileUserSelect,
  }) as Promise<ProfileUser | null>;
