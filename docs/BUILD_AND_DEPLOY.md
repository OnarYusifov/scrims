...
You can use any Turborepo-compatible cache backend (Vercel, S3 proxy, etc). Populate the following environment variables wherever the pipeline runs (local opt-in, CI, Dokploy builds):
...
## 3. Nixpacks / Dokploy Changes

### 3.1 Unified Configuration (`nixpacks.toml`)

The unified nixpacks configuration builds both backend and frontend in a single service:

- Uses Node 22 with build-essential libs for `canvas` (needed for Discord bot leaderboard rendering).
- Runs `npm ci` to install all workspace dependencies.
- Generates Prisma client with `npx prisma generate`.
- Builds both apps via `turbo run build --filter=@trayb/backend --filter=@trayb/frontend`.
- Prunes dev dependencies after the build (`npm prune --omit=dev`).
- Caches `node_modules`, `apps/backend/node_modules`, `apps/frontend/node_modules`, `.turbo`.
- Starts both services with `npm run start` (backend on port 4001, frontend on port 4000).

> **Dokploy setup:** point the unified service to `nixpacks.toml`. Add the `TURBO_*` env vars, `NODE_ENV=production`, and (optionally) `TURBO_REMOTE_CACHE_SIGNATURE_KEY`. Pruning dev deps reduces cold-start size; ensure the build output includes everything your start command needs before pruning.
...
3. **Dokploy:** redeploy the unified service. Confirm build logs show `turbo run build` and `npm prune` executing.
4. **Runtime:** `Dokploy → Logs` ensure both `node dist/index.js` and `next start` boot successfully, no missing module errors.
...
- Add secrets to GitHub + Dokploy once provider is ready.
- Expand test tasks to real unit/integration suites (will make `npm run test` meaningful).
- Optionally add Dokploy post-deploy script to warm Next.js ISR pages.
